"""Flask service for managing undetected Chrome Selenium sessions.

Provides endpoints to open and close browser sessions using a local
undetected-chromedriver instance with custom logging and options.
"""
...
def configure_logging() -> None:
    """Configure the root logger with colored output and support for a custom TRACE level.
    ...
    logger.trace("Logging is configured")

if __name__ == "__main__":
    """
    Entry point for running the Flask app with Waitress.

    Starts the server on 127.0.0.1:5000 and configures logging beforehand.
    """
    configure_logging()
    logger.info("Starting Waitress server to handle Python Selenium sessions")
